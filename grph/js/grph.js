...
function number_formatter(field){
  ...
  var space = (unit === '%')? "" : " ";
  //TODO improve handling of unit etc.

  return function(value){
    return (factor*value).toLocaleString() + space + unit || "-";
  };
}
  ...
  var graph = grph_generic_graph(axes, dispatch, "line", function(g, data) {
    ...
    for (var i = 0; i < d.length; ++i){
      ...
      g.append("path")
        ...
        .attr("d", line(d[i].values[0]))
        .datum(d[i])
      .transition()
        .duration(1000)
        .attrTween('d', pathTween.bind(this, i));
    }
